...
namespace StratSim.ViewModel
{
    ...
    static class MyEvents
    {
        ...
        /// </summary>
        /// <param name="positions">The array of race positions as generated by the race</param>
        /// <param name="lapNumber">The lap on which the intervals are being updated. Lap number is zero based.</param>
        public static void OnUpdateIntervals(Model.racePosition[,] positions, int lapNumber)
        {
            UpdateIntervals(positions, lapNumber + 1);
        }

        public delegate void UpdateIntervalDataEventHandler(Model.racePosition[,] positions, int lapNumber);
        public static event UpdateIntervalDataEventHandler UpdateIntervals;

        /// <summary>
        /// Fires the LapNumberChanged event, causing the graph to zoom in on a particular width
        /// </summary>
        /// <param name="oldLapNumber">The original lap number</param>
        /// <param name="newLapNumber">The new lap number</param>
        public static void OnLapNumberChanged(int newLapNumber)
        {
            LapNumberChanged(newLapNumber);
            UpdateIntervals(Model.Data.Race.Positions, newLapNumber);
        }

        public delegate void LapNumberChangedEventHandler(int newLapNumber);
        ...
        /// </summary>
        public static void OnStrategyModificationsComplete(bool showAllOnGraph, bool changeNormalisedDriver)
        {
            if (StrategyModificationsComplete != null)
                StrategyModificationsComplete(showAllOnGraph, changeNormalisedDriver);
        }

        public delegate void StrategyModificationsCompleteEventHandler(bool showAllOnGraph, bool changeNormalisedDriver);
        public static event StrategyModificationsCompleteEventHandler StrategyModificationsComplete;

        public delegate void StrategyModifiedEventHandler(int driverIndex, Model.Strategy strategy, bool showAllOnGraph);
