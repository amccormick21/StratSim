...
namespace MyFlowLayout
{
    ...
    public class WindowFlowPanel : Panel
    {
        ...
        {
            this.BackgroundImage = Properties.Resources.Screen_Background;
        }

        public void LinkToForm(MainForm Form)
        {
            this.ParentForm = Form;

            //Locate this panel on the form
            Location = new Point(0, Form.header.Height);
            Size = new Size(Form.ClientRectangle.Width, Form.ClientSize.Height - Form.header.Height);
            ...
            IOController.FlowLayoutEvents.MainPanelLayoutChanged += MyEvents_MainPanelLayoutChanged;
        }
        ...
        {
            this.Size = new Size(parent.ClientSize.Width, parent.ClientSize.Height - parent.header.Height);
        ...
        public void AddControl(IDockableControl controlToAdd, bool notAlreadyOnForm)
        {
            ...
            UpdatePanelLists(controlToAdd, notAlreadyOnForm, controlCopy);
            UpdateScreenOutput(controlToAdd, controlCopy);
        }

        /// <summary>
        /// Adds the specified control to the graphical output on the panel.
        /// </summary>
        /// <param name="controlToAdd">The IDockableControl to add to the panel</param>
        /// <param name="controlCopy">A control copy of the panel</param>
        void UpdateScreenOutput(IDockableControl controlToAdd, Control controlCopy)
        {
        ...
        /// <summary>
        /// Updates the lists of displayed panels and visible controls on the panel.
        /// </summary>
        /// <param name="controlToAdd">The IDockableControl to add to the panel</param>
        /// <param name="notAlreadyOnForm">Boolean specifying whether the panel is already on the form.</param>
        /// <param name="controlCopy">A control copy of the panel.</param>
        void UpdatePanelLists(IDockableControl controlToAdd, bool notAlreadyOnForm, Control controlCopy)
        {
            if (notAlreadyOnForm) //if the panel is not already on the form, add it to the form
            {
                dockableControlsInPanel.Add(controlToAdd);

                if (controlToAdd.Type == typeof(MyPanel)) //if a panel is being added
                {
        ...
        void ResizeUsingLayoutLines()
        {
            ...
            IOController.FlowLayoutEvents.OnMainPanelResize();
        }
