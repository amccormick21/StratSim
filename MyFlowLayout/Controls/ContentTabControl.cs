...
namespace MyFlowLayout
{
    public class ContentTabControl : MyPanel
    {
        ...
        void ContentTabControl_OpenedInNewForm(MainForm NewForm)
        {
            ...
            {
                p.ParentForm = NewForm;
            }
            NewForm.IOController.ContentTabControl = this;
        }

        void MainControls_ControlRemoved(object sender, ControlEventArgs e)//TO Remove
        {
            MainControls.SelectedIndex = MainControls.TabCount - 1;
        }

        void MainControls_ControlAdded(object sender, ControlEventArgs e)//TO remove
        {
            MainControls.SelectedIndex = MainControls.TabCount - 1;
        }

        internal override void MyPanel_PanelOpened(MainForm OpenedOnForm)
        {
            base.MyPanel_PanelOpened(OpenedOnForm);
            IOController.FlowLayoutEvents.MainPanelResizeEnd += ResizeMainControl;
        }
        internal override void MyPanel_PanelClosed(MainForm LeavingForm)
        {
            base.MyPanel_PanelClosed(LeavingForm);
            IOController.FlowLayoutEvents.MainPanelResizeEnd -= ResizeMainControl;
        }
